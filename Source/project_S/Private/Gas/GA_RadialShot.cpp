...
UGA_RadialShot::UGA_RadialShot()
{
    ...
    SetAssetTags(FGameplayTagContainer(FGameplayTag::RequestGameplayTag(TEXT("Ability.Attack.RadialShot"))));
}
...
void UGA_RadialShot::OnGiveAbility(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& Spec)
{
	...
	if (!ActorInfo || !ActorInfo->IsNetAuthority() || !Spec.Handle.IsValid())
	{
		return;
	}
	UE_LOG(LogTemp, Warning, TEXT("[온기브 호출]"));
	UE_LOG(LogTemp, Warning, TEXT("Spec.Handle : %s"), *Spec.Handle.ToString());
	CachedASC = ActorInfo->AbilitySystemComponent;
	AutoCastSpecHandles.AddUnique(Spec.Handle);
	SkillStack = AutoCastSpecHandles.Num();
	StopAutoCastTimer();
	StartAutoCastTimer();
}
...
void UGA_RadialShot::OnRemoveAbility(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& Spec)
{
	...
	AutoCastSpecHandles.Remove(Spec.Handle);
	SkillStack = AutoCastSpecHandles.Num();
	StopAutoCastTimer();
